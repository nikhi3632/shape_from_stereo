    ...
    for test_case in test_cases:
        try:
            ...
            if expected:
                ...
                print('Correct output. {} and {}'.format(
                    test_case['actual'], test_case['expected']))
...
def main():
    ...
    if len(argv) > 1:
        if argv[1] == 'dmap':
            ...
            if distance_map_result:
                print("All OK for dmap")
                sys.exit(0)
            else:
                print("Some tests FAILED for dmap")
                sys.exit(1)
        elif argv[1] == 'qtree':
            quadtree_result = test_quadtree()
            if quadtree_result:
                print("All OK for qtree")
                sys.exit(0)
            else:
                print("Some tests FAILED for qtree")
                sys.exit(1)
        else:
